...
function MainPageNavbar() {
  ...
  useEffect(() => {
    const getProfile = () => {
      const profile = supabase.auth.getUser();
          ...
          <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
            <div className="flex space-x-4">
              <a onClick={login} className="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white" style={{ "cursor": "pointer" }}>
                <FontAwesomeIcon icon={faSignIn} className="text-white" onClick={mute} />
              </a>
              ...
              {user ?
                <div>
                  <div className="relative inline-block text-left">
                    <div>
                      <button type="button" className="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white" style={{ "cursor": "pointer" }} id="menu-button" aria-expanded="true" aria-haspopup="true">
                    ...
                    <div className="absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabIndex="-1">
                      <div className="py-1" role="none">
                        <a href="/profile" className="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabIndex="-1" id="menu-item-0">Account settings</a>
